...
function EditContact({ onClose, onEditContact, existingContact }) {
  const [statusColor, setStatusColor] = useState("");
  const [showStatus, setShowStatus] = useState(false);
  const [statusLabel, setStatusLabel] = useState("");
  const [imagePreview, setImagePreview] = useState(
    existingContact.image || null
  );
  ...
  useEffect(() => {
    // Initialize initialStatus and set the form data
    const initialStatus = existingContact.status || "None"; // Default to "None"
    setStatusLabel(initialStatus);
    setStatusColor(
      statusOptions.find((status) => status.label === initialStatus)?.color ||
        "none"
    );
    ...
    setFormData({
      ...
      image: existingContact.image || null,
